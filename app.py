"""MTG tuckbox generator with dark mode and collapsible sizing controls."""
...
class Box:
    ...
    def panels(self):
        ...
        return {"back": (x, y, self.panel_d, self.panel_h),
    ...
    def flaps(self):
        ...
        main, dust, shoulder = c.bottom_depth, c.dust_depth, min(6, c.panel_d / 7)
        front, left, right = x+c.panel_d+c.panel_w, x+c.panel_d, x+2*c.panel_d+c.panel_w
        return [
            ...
            ([(left+3,y),(left+c.panel_w-3,y),(left+c.panel_w,y-dust+4),(left+c.panel_w-4,y-dust),(left+4,y-dust),(left,y-dust+4)], "dust"),
            ([(right+3,y),(right+c.panel_w-3,y),(right+c.panel_w,y-dust+4),(right+c.panel_w-4,y-dust),(right+4,y-dust),(right,y-dust+4)], "dust"),
            ...
            ([(left+3,y+c.panel_h),(left+c.panel_w-3,y+c.panel_h),(left+c.panel_w-4,y+c.panel_h+dust),(left+4,y+c.panel_h+dust)], "dust"),
            ([(right+3,y+c.panel_h),(right+c.panel_w-3,y+c.panel_h),(right+c.panel_w-4,y+c.panel_h+dust),(right+4,y+c.panel_h+dust)], "dust"),
        ]
...
class App(ttk.Window):
    def __init__(self):
        ...
        self.dark = False
        self.box = Box(); self.art = {"front": None, "back": None}; self.offset = {"front": [0., 0.], "back": [0., 0.]}
        self.photos = []; self.drag = None; self.scale = 1.; self.origin = (0., 0.)
        self.vars = {k: ttk.StringVar(value=str(v)) for k, v in {"cards":22,"card_w":63,"card_h":88,"thickness":.30,"clearance":2,"bleed":3,"glue":12,"tuck":25}.items()}
    ...
    def build_ui(self):
        ...
        self.left = ttk.Frame(root, width=315); self.left.pack(side=LEFT, fill=Y, padx=(0,12)); self.left.pack_propagate(False)
        right = ttk.Frame(root); right.pack(side=RIGHT, fill=BOTH, expand=True)
        header = ttk.Frame(self.left); header.pack(fill=X, pady=(0,10))
        ...
        self.size_box = ttk.Labelframe(self.left, text="Sizing options", padding=8, bootstyle="primary")
        self.size_box.pack(fill=X, pady=(0,8))
        self.size_toggle = ttk.Button(self.left, text="Hide sizing options", command=self.toggle_sizing, bootstyle="link")
        self.size_toggle.pack(anchor="w", pady=(0,5))
        labels = {"cards":"Cards per pack", "card_w":"Card width (mm)", "card_h":"Card height (mm)", "thickness":"Thickness/card (mm)", "clearance":"Clearance (mm)", "bleed":"Bleed (mm)", "glue":"Glue tab (mm)", "tuck":"Tuck flap depth (mm)"}
        ...
        ttk.Labelframe(self.left, text="Artwork placement", padding=8, bootstyle="secondary").pack(fill=X, pady=6)
        artbox = self.left.winfo_children()[-1]
        ttk.Button(artbox, text="Load front artwork", command=lambda:self.load_art("front"), bootstyle="secondary").pack(fill=X,pady=2)
        ttk.Button(artbox, text="Load back artwork", command=lambda:self.load_art("back"), bootstyle="secondary").pack(fill=X,pady=2)
        ttk.Button(artbox, text="Reset artwork positions", command=self.reset_art, bootstyle="link").pack(fill=X,pady=2)
        self.art_label = ttk.Label(artbox, text="No artwork loaded", wraplength=285); self.art_label.pack(anchor="w", pady=4)
        ttk.Label(artbox, text="Drag artwork inside its panel in the preview.", wraplength=285).pack(anchor="w")

        sheet = ttk.Labelframe(self.left, text="Print sheet", padding=8, bootstyle="secondary"); sheet.pack(fill=X,pady=6)
        ...
        self.info=ttk.Label(self.left,justify="left",wraplength=295); self.info.pack(anchor="w",pady=12)
    ...
    def toggle_theme(self):
        ...
        self.canvas.config(bg="#20242a" if self.dark else "#edf0f2")
        self.redraw()
    ...
    def load_art(self, side):
        ...
        self.art[side]=path; self.offset[side]=[0.,0.]; self.art_label.config(text=f"Front: {Path(self.art['front']).name if self.art['front'] else 'none'}\nBack: {Path(self.art['back']).name if self.art['back'] else 'none'}"); self.redraw()

    def reset_art(self): self.offset={"front":[0.,0.],"back":[0.,0.]}; self.redraw()
    ...
    def panel_rect(self,side):
        x,y,w,h=self.box.panels()[side]; ox,oy=self.offset[side]; return x+ox,y+oy,w,h
    ...
    def redraw(self):
        ...
        def poly(points,fill): self.canvas.create_polygon([n for p in points for n in self.page_xy(*p)],fill=fill,outline="#111",width=2)
        for x,y,w,h,name in b.all_panels():poly([(x,y),(x+w,y),(x+w,y+h),(x,y+h)],"#fff" if name in ("front","back") else "#f4f5f6")
        for points,_ in b.flaps():poly(points,"#f8f8f8")
        for x,y,w,h,name in b.all_panels():a,bb=self.page_xy(x+w/2,y+h/2);self.canvas.create_text(a,bb,text=name.upper(),fill="#555",font=("Segoe UI",10,"bold"))
        for x in (b.glue,b.glue+b.panel_d,b.glue+b.panel_d+b.panel_w,b.glue+2*b.panel_d+b.panel_w):a,bb=self.page_xy(x,0);c,d=self.page_xy(x,b.page_h);self.canvas.create_line(a,bb,c,d,fill="#2865ad",dash=(6,4))
        for y in (b.body_y,b.body_y+b.panel_h):a,bb=self.page_xy(0,y);c,d=self.page_xy(b.page_w,y);self.canvas.create_line(a,bb,c,d,fill="#2865ad",dash=(6,4))
        self.draw_art("front");self.draw_art("back")
        self.info.config(text=f"Cards per pack: {b.cards}\nInside: {b.panel_w:.2f} W × {b.panel_d:.2f} D × {b.panel_h:.2f} H mm\nDieline: {b.cut_w:.2f} × {b.cut_h:.2f} mm\n{'Dark mode' if self.dark else 'Light mode'} • Print at 100%")

    def draw_art(self,side):
        path=self.art[side]
        if not path or Image is None:return
        try:
            x,y,w,h=self.panel_rect(side)
            with Image.open(path) as source: image=ImageOps.exif_transpose(source).convert("RGB")
            image=ImageOps.fit(image,(max(2,round(w*self.scale)),max(2,round(h*self.scale))),method=Image.Resampling.LANCZOS)
            photo=ImageTk.PhotoImage(image);self.photos.append(photo);a,bb=self.page_xy(x+w/2,y+h/2);self.canvas.create_image(a,bb,image=photo,anchor="center")
        except Exception as exc:self.info.config(text=f"Artwork preview failed: {exc}")

    def start_drag(self,event):
        for side,path in self.art.items():
            if not path:continue
            x,y,w,h=self.panel_rect(side);a,bb=self.page_xy(x+w/2,y+h/2)
            if abs(event.x-a)<=w*self.scale/2 and abs(event.y-bb)<=h*self.scale/2:self.drag=(side,event.x,event.y);return
    def move_drag(self,event):
        if not self.drag:return
        side,px,py=self.drag;self.offset[side][0]+=(event.x-px)/self.scale;self.offset[side][1]-=(event.y-py)/self.scale;self.drag=(side,event.x,event.y);self.redraw()

    def svg_image(self,side,H):
        path=self.art[side]
        if not path:return ""
        x,y,w,h=self.panel_rect(side);o=self.box.bleed;data=base64.b64encode(Path(path).read_bytes()).decode("ascii");mime=mimetypes.guess_type(path)[0] or "image/png"; clip=f"clip-{side}"
    ...
    def save_svg(self):
        ...
        try:
            ...
            def poly(points,stroke="#111",dash=""):out.append(f'<polygon points="{" ".join(f"{x+o},{H-y-o}" for x,y in points)}" fill="none" stroke="{stroke}" stroke-width=".25"'+(f' stroke-dasharray="{dash}"' if dash else '')+'/>' )
            ...
            for x in (b.glue,b.glue+b.panel_d,b.glue+b.panel_d+b.panel_w,b.glue+2*b.panel_d+b.panel_w):out.append(f'<path d="M{x+o} {H-o}V{H-b.page_h+o}" stroke="#2865ad" stroke-width=".2" stroke-dasharray="2,1"/>')
            for y in (b.body_y,b.body_y+b.panel_h):out.append(f'<path d="M{o} {H-y-o}H{W-o}" stroke="#2865ad" stroke-width=".2" stroke-dasharray="2,1"/>')
    ...
    def save_pdf(self,pagesize):
        ...
        try:
            ...
            def draw_one(px,py):
                ...
                def polygon(points):q=pdf.beginPath();q.moveTo(*P(*points[0]));[q.lineTo(*P(*v)) for v in points[1:]];q.close();pdf.setStrokeColorRGB(.08,.08,.08);pdf.setDash();pdf.drawPath(q,stroke=1,fill=0)
                for x,y,w,h,_ in b.all_panels():polygon([(x,y),(x+w,y),(x+w,y+h),(x,y+h)])
                for points,_ in b.flaps():polygon(points)
                if ImageReader:
                    for side in ("back","front"):
                        if self.art[side]:
                            x,y,w,h=self.panel_rect(side);pdf.drawImage(ImageReader(self.art[side]),*P(x,y),width=w*mm*scale,height=h*mm*scale,preserveAspectRatio=False,mask="auto")
