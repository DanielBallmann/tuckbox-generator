"""Straight-tuck-end MTG box generator.

The app lays out a single cut sheet for a straight-tuck box: four body panels,
a top flap, a bottom tuck flap, a glue tab, and chamfered dust flaps. Artwork
is assigned to the actual box faces so the preview and exported PDF/SVG remain
aligned with the same foldable layout.
"""
...
@dataclass
class Box:
    """Physical box dimensions and cut-sheet layout in millimetres."""
    ...
    def panels(self):
        """Return all rectangular faces and flaps in cut-sheet coordinates."""
    ...
    def body(self):
        """Return the four body panels used for artwork placement."""
        return [
            (x, y, w, h, name)
    ...
    def cut_polygons(self):
        """Generate the cut lines for the tuck flap and dust-flap geometry."""
        p = self.panels()
        x, y, w, d, h = self.glue, self.body_y, self.width, self.depth, self.height
        dust = min(d * 0.42, h * 0.38)
        left = p["side_left"][0]
        right = p["side_right"][0]
        ...
        dust_polys = [
            ...
            [(left + 2, y + h), (left + d - 2, y + h), (left + d - 4, y + h + dust), (left + 4, y + h + dust)],
            [(right + 2, y + h), (right + d - 2, y + h), (right + d - 4, y + h + dust), (right + 4, y + h + dust)],
        ]
        return [top, bottom, *dust_polys]


class App(BaseWindow):
    """Tkinter UI, preview renderer, and export logic."""
    ...
    def __init__(self):
        ...
        self.vars = {
            key: ttk.StringVar(value=str(value))
            for key, value in {
                "cards": 22,
                "card_w": 63,
                "card_h": 88,
    ...
    def build_ui(self):
        ...
        labels = {
            ...
            "card_w": "Card width (mm)",
            "card_h": "Card height (mm)",
            "thickness": "Thickness/card (mm)",
            "clearance": "Clearance (mm)",
            "bleed": "Bleed (mm)",
            "glue": "Glue tab (mm)",
            "tuck_lip": "Tuck lip (mm)",
            "notch": "Thumb notch (mm)",
        }
        for key, var in self.vars.items():
            row = ttk.Frame(self.size_box)
        ...
        self.size_button = ttk.Button(side, text="Show sizing options", command=self.toggle_size, bootstyle="link")
        self.size_button.pack(anchor="w", pady=(0, 5))

        art = ttk.Labelframe(side, text="Artwork — fixed to faces", padding=8, bootstyle="secondary")
        ...
        for label, face in (
            ...
            self.register_drop(button, face)

        self.art_label = ttk.Label(art, text="No artwork loaded", wraplength=290)
        self.art_label.pack(anchor="w", pady=4)
        ttk.Label(art, text="Drop an image onto the matching button. JPG/JPEG, spaces, and Windows file URLs are supported.", wraplength=290).pack(anchor="w")

        sheet = ttk.Labelframe(side, text="Print sheet", padding=8, bootstyle="secondary")
        sheet.pack(fill=X, pady=6)
        ...
        self.info = ttk.Label(side, justify="left", wraplength=300)
        ...
        self.canvas = tk.Canvas(view, bg="#edf0f2", highlightthickness=1)
        self.canvas.pack(fill=BOTH, expand=True)
        self.canvas.bind("<Configure>", lambda _e: self.redraw())

    def register_drop(self, widget, face):
        """Register each button as a drop target when tkinterdnd2 is installed."""
    ...
    def normalize_drop_path(self, value):
        """Normalize regular paths, file:// URLs, and Windows drive-letter paths."""
        if value is None:
            return None
    ...
    def drop_art(self, face, data):
        """Accept the first valid dropped file and assign it to the target face."""
        try:
            pieces = list(self.tk.splitlist(str(data)))
        except tk.TclError:
            pieces = [str(data)]
        ...
        for raw in pieces:
            cleaned = self.normalize_drop_path(raw)
            if cleaned:
                self.set_art(face, cleaned)
                return
    ...
    def set_art(self, face, path):
        """Validate the file and accept only real image files."""
        path_obj = Path(path).expanduser()
        if path_obj.suffix.lower() not in SUPPORTED:
            messagebox.showerror("Unsupported artwork", "Choose a JPG, PNG, WEBP, BMP, GIF, or TIFF image.")
            return
        if not path_obj.is_file():
            messagebox.showerror("Artwork not found", f"The dropped file does not exist:\n{path_obj}")
            return
        if Image is None:
            messagebox.showerror("Missing Pillow", "Install dependencies with: pip install -r requirements.txt")
            return
        ...
        self.update_art_label()
        self.redraw()

    def update_art_label(self):
        label = "\n".join(f"{face}: {Path(path).name}" for face, path in self.art.items() if path)
        self.art_label.config(text=label or "No artwork loaded")

    def toggle_size(self):
        if self.size_visible:
            self.size_box.pack_forget()
            self.size_button.config(text="Show sizing options")
        else:
            self.size_box.pack(fill=X, before=self.size_button, pady=(0, 8))
            self.size_button.config(text="Hide sizing options")
        self.size_visible = not self.size_visible

    def toggle_theme(self):
        self.dark = not self.dark
        self.style.theme_use("darkly" if self.dark else "flatly")
        self.canvas.configure(bg="#1f2329" if self.dark else "#edf0f2")
        self.redraw()
    ...
    def apply(self):
        try:
            ...
            previous = self.art
            self.box = Box(**values)
            self.art = {face: previous.get(face) for face in self.box.panels()}
            self.update_art_label()
            self.redraw()
    ...
    def page_xy(self, x, y):
        return (
            self.origin[0] + (x + self.box.bleed) * self.scale,
            self.origin[1] + (self.box.sheet_h - y - self.box.bleed) * self.scale,
        )
    ...
    def redraw(self):
        ...
        self.scale = min(
            max(self.canvas.winfo_width() - 30, 100) / b.sheet_w,
            max(self.canvas.winfo_height() - 30, 100) / b.sheet_h,
        )
        self.origin = (15.0, 15.0)

        cut = "#ff6675" if self.dark else "#dc3545"
        score = "#adb5bd" if self.dark else "#6c757d"
        panel = "#30343b" if self.dark else "#ffffff"
        side = "#272b31" if self.dark else "#f0f2f4"

        def poly(points, fill, outline=cut):
            coords = [coord for point in points for coord in self.page_xy(*point)]
        ...
        for points in b.cut_polygons():
            poly(points, side)

        # Fold lines delineate the visible body seams and tuck-flap hinges.
        seam_x = (b.glue, b.glue + b.width, b.glue + b.width + b.depth, b.glue + 2 * b.width + b.depth)
        for x in seam_x:
            a, bb = self.page_xy(x, 0)
            c, d = self.page_xy(x, b.sheet_h)
            self.canvas.create_line(a, bb, c, d, fill=score, dash=(7, 4), width=2)

        for y in (b.body_y, b.body_y + b.height):
            a, bb = self.page_xy(0, y)
            c, d = self.page_xy(b.sheet_w, y)
            self.canvas.create_line(a, bb, c, d, fill=score, dash=(7, 4), width=2)

        for face, path in self.art.items():
            self.draw_art(face, path)

        self.info.config(
            text=(
                f"Cards per pack: {b.cards}\n"
                f"Internal: {b.width:.1f} W × {b.depth:.1f} D × {b.height:.1f} H mm\n"
                f"Straight-tuck sheet: {b.sheet_w:.1f} × {b.sheet_h:.1f} mm\n"
                f"Artwork faces: {sum(bool(p) for p in self.art.values())}/6"
            )
        )

    def draw_art(self, face, path):
        """Draw a fresh EXIF-corrected RGB image so JPG orientation is safe."""
        ...
        try:
            ...
            with Image.open(path) as source:
                image = ImageOps.fit(
                    ...
                    (max(2, round(w * self.scale)), max(2, round(h * self.scale))),
                    method=Image.Resampling.LANCZOS,
                )
            photo = ImageTk.PhotoImage(image)
            self.photos.append(photo)
            cx, cy = self.page_xy(x + w / 2, y + h / 2)
    ...
    def svg_art(self, face):
        ...
        return (
            f'<image href="data:{mime};base64,{data}" '
            f'x="{x + self.box.bleed}" y="{self.box.sheet_h - y - self.box.bleed - h}" '
    ...
    def save_svg(self):
        ...
        def polygon(points):
            pts = " ".join(f"{x + o},{H - y - o}" for x, y in points)
            out.append(f'<polygon points="{pts}" fill="none" stroke="#dc3545" stroke-width=".25"/>')

        for x, y, w, h, _ in b.body():
            polygon([(x, y), (x + w, y), (x + w, y + h), (x, y + h)])
        for points in b.cut_polygons():
            polygon(points)
        out.extend(self.svg_art(face) for face in self.art)

        for x in (b.glue, b.glue + b.width, b.glue + b.width + b.depth, b.glue + 2 * b.width + b.depth):
            out.append(f'<path d="M{x + o} {H - o}V{H - b.sheet_h + o}" fill="none" stroke="#777" stroke-dasharray="2,1"/>')
        for y in (b.body_y, b.body_y + b.height):
            out.append(f'<path d="M{o} {H - y - o}H{W - o}" fill="none" stroke="#777" stroke-dasharray="2,1"/>')
    ...
    def save_pdf(self, pagesize):
        if Canvas is None or ImageReader is None:
            messagebox.showerror("Missing dependency", "Install dependencies with: pip install -r requirements.txt")
            return
        ...
        try:
            ...
            if cols < 1 or rows < 1 or gap < 0:
                raise ValueError

            b = self.box
            pw, ph = pagesize
            margin = 12 * mm
            sw = cols * b.sheet_w + (cols - 1) * gap
            sh = rows * b.sheet_h + (rows - 1) * gap
            scale = min((pw - 2 * margin) / (sw * mm), (ph - 2 * margin) / (sh * mm))
            left = margin + (pw - 2 * margin - sw * mm * scale) / 2
            bottom = margin + (ph - 2 * margin - sh * mm * scale) / 2
            pdf = Canvas(path, pagesize=pagesize)

            def draw_one(px, py):
                def P(x, y):
                    return px + (x + b.bleed) * mm * scale, py + (y + b.bleed) * mm * scale
                ...
                def outline(points):
                    ...
                    for point in points[1:]:
                        q.lineTo(*P(*point))
                    q.close()
                    pdf.setStrokeColorRGB(0.86, 0.10, 0.18)
                    pdf.setDash()
                ...
                pdf.setDash(3, 2)
                for x in (b.glue, b.glue + b.width, b.glue + b.width + b.depth, b.glue + 2 * b.width + b.depth):
                    pdf.line(*P(x, 0), *P(x, b.sheet_h))
                for y in (b.body_y, b.body_y + b.height):
                    pdf.line(*P(0, y), *P(b.sheet_w, y))
                pdf.setDash()

            for row in range(rows):
                for col in range(cols):
                    draw_one(
                        left + col * (b.sheet_w + gap) * mm * scale,
                        bottom + (rows - 1 - row) * (b.sheet_h + gap) * mm * scale,
                    )
