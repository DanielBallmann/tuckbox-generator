"""Straight-tuck-end trading-card box generator.

The six artwork faces are the four body panels plus the attached top and bottom
flaps. Artwork is fixed to those faces; files can be selected or dropped onto
the matching artwork button when tkinterdnd2 is installed.
"""
...
class Box:
    ...
    def panels(self):
        """Physical faces in cut-sheet coordinates, y measured from bottom."""
        ...
        return {
            ...
            # The top flap is attached to the back panel.
            "top": (x, y + h, w, d),
            # The bottom flap is attached to the front panel and folds under.
    ...
    def cut_polygons(self):
        """Cut outlines: top/bottom flaps plus four chamfered dust flaps."""
        p = self.panels()
        x, y, w, d, h = self.glue, self.body_y, self.width, self.depth, self.height
        dust = min(d * .42, h * .38)
        left = p["side_left"][0]
        ...
        top = [(p["top"][0], p["top"][1]),
               ...
               (p["top"][0] + w / 2 + self.notch / 2, p["top"][1] + self.tuck_lip),
               (p["top"][0] + w / 2 + self.notch / 2, p["top"][1] + self.tuck_lip - 4),
               (p["top"][0] + w / 2 - self.notch / 2, p["top"][1] + self.tuck_lip - 4),
               (p["top"][0] + w / 2 - self.notch / 2, p["top"][1] + self.tuck_lip),
               (p["top"][0] + 7, p["top"][1] + self.tuck_lip)]
        bottom = [(p["bottom"][0], p["bottom"][1]),
...
class App(BaseWindow):
    ...
    def build_ui(self):
        ...
        labels = {"cards":"Cards per pack", "card_w":"Card width (mm)", "card_h":"Card height (mm)",
                  "thickness":"Thickness/card (mm)", "clearance":"Clearance (mm)", "bleed":"Bleed (mm)",
                  "glue":"Glue tab (mm)", "tuck_lip":"Tuck lip (mm)", "notch":"Thumb notch (mm)"}
        ...
        labels_faces = (("Back", "back"), ("Left side", "side_left"), ("Front", "front"),
                        ("Right side", "side_right"), ("Top flap", "top"), ("Bottom flap", "bottom"))
        for label, face in labels_faces:
        ...
        ttk.Label(art, text="Drop an image onto a matching button. Artwork is fixed and cannot be dragged.", wraplength=290).pack(anchor="w")
        ...
        for label, var in (("Columns", self.cols), ("Rows", self.rows), ("Gap (mm)", self.gap)):
            row = ttk.Frame(sheet); row.pack(fill=X, pady=2)
            ttk.Label(row, text=label, width=20).pack(side=LEFT); ttk.Entry(row, textvariable=var, width=10).pack(side=RIGHT)
    ...
    def redraw(self):
        ...
        self.photos.clear()
        self.scale = min(max(self.canvas.winfo_width() - 30, 100) / b.sheet_w,
        ...
        def poly(points, fill, outline=cut):
            coords = [n for p in points for n in self.page_xy(*p)]
        ...
        for poly_points in b.cut_polygons():
            poly(poly_points, side)

        # score lines at every body seam, including the top and bottom fold seams
        seam_x = (b.glue, b.glue+b.width, b.glue+b.width+b.depth, b.glue+2*b.width+b.depth)
        for x in seam_x:
            a, bb = self.page_xy(x, 0); c, d = self.page_xy(x, b.sheet_h)
            self.canvas.create_line(a, bb, c, d, fill=score, dash=(7, 4), width=2)
        for y in (b.body_y, b.body_y+b.height):
            a, bb = self.page_xy(0, y); c, d = self.page_xy(b.sheet_w, y)
            self.canvas.create_line(a, bb, c, d, fill=score, dash=(7, 4), width=2)
        ...
        for x, y in ((0, 0), (b.cut_w, 0), (0, b.cut_h), (b.cut_w, b.cut_h)):
            a, bb = self.page_xy(x, y)
            self.canvas.create_line(a-5, bb, a+5, bb, fill=cut)
            self.canvas.create_line(a, bb-5, a, bb+5, fill=cut)
    ...
    def save_pdf(self, pagesize):
        ...
        try:
            ...
            if cols < 1 or rows < 1 or gap < 0:
                raise ValueError
            b = self.box; pw, ph = pagesize; margin = 12 * mm
            sw = cols*b.sheet_w + (cols-1)*gap; sh = rows*b.sheet_h + (rows-1)*gap
            ...
            def draw_one(px, py):
                ...
                for face, image_path in self.art.items():
                    if image_path:
                        ...
                        pdf.drawImage(ImageReader(image_path), *P(x, y), width=w*mm*scale, height=h*mm*scale, preserveAspectRatio=False, mask="auto")
            ...
            for row in range(rows):
                for col in range(cols):
                    draw_one(left+col*(b.sheet_w+gap)*mm*scale, bottom+(rows-1-row)*(b.sheet_h+gap)*mm*scale)
